...
<head>
    ...
    <title>Operator Accounts | Admin Inventaris</title>
            ...
            <div class="space-y-1">
                ...
                <div class="pl-8 flex flex-col gap-2 mt-2">
                    ...
                    <a href="{{ route('users.staff') }}"
                        ...
                        <span class="w-1.5 h-1.5 bg-white rounded-full"></span> Operator
                    </a>
                </div>
            </div>
        </nav>
    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="bg-white shadow-sm px-8 py-4 flex justify-between items-center border-b">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-lg font-semibold text-gray-700">Operator Accounts Management</h2>
            </div>
        ...
        <div class="p-8">
            ...
            <div
                class="mb-6 flex items-center p-4 text-emerald-800 border-l-4 border-emerald-500 bg-emerald-50 shadow-md rounded-r-lg">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm">Password untuk user ini adalah:
                        <span
            ...
            <div
                class="mb-6 flex items-center p-4 text-amber-800 border-l-4 border-amber-500 bg-amber-50 shadow-md rounded-r-lg">
                ...
                <div class="ml-3">
                    <p class="text-sm font-bold">Password Berhasil Direset!</p>
                    <p class="text-sm">Password baru adalah:
                        <span
                            class="bg-white px-2 py-0.5 rounded border border-amber-300 font-mono font-bold text-amber-600">
            ...
            <div class="mb-6 flex justify-between items-end">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Operator Accounts Table</h1>
                    <p class="text-gray-500 text-sm">Add, delete, reset <span
                            class="text-pink-500">.staff-accounts</span></p>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('users.staff.export') }}"
                        class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-semibold flex items-center gap-2 shadow-md transition">
                        Export Excel
                    </a>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 flex justify-center gap-2">
                                ...
                                <form action="{{ route('users.reset_password', $user->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="bg-amber-400 text-white px-4 py-1.5 rounded-md text-sm hover:bg-amber-500 transition shadow-sm font-medium">
                                        Reset Password
                                    </button>
                                </form>

                                <button type="button" onclick="openDeleteModal('{{ route('users.destroy', $user->id) }}')" class="bg-red-500 text-white px-4 py-1.5 rounded-md text-sm hover:bg-red-600 transition shadow-sm font-medium">
                                    Delete
                                </button>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div class="relative bg-white rounded-xl shadow-lg max-w-md w-full p-6 text-center">
                ...
                <p class="text-gray-500 mb-6">Apakah Anda yakin ingin menghapus akun operator ini? Tindakan ini tidak
                    dapat dibatalkan.</p>
                <div class="flex justify-center gap-4">
                    <button onclick="closeDeleteModal()"
                        class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg font-semibold transition">Batal</button>
                    <form id="deleteForm" action="" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-semibold shadow-md transition">Ya,
                            Hapus</button>
                    </form>
                </div>
            </div>
